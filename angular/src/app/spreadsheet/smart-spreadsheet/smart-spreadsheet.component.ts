...
import { getAzureChatAIRequest } from '../.././ai-models/azure_openai';
import { SheetModel, Spreadsheet, getCell, FindOptions,SpreadsheetAllModule } from '@syncfusion/ej2-angular-spreadsheet';
...
public sheet: SheetModel[] = [
  ...
  public created(){
    ...
    (this.spreadsheet as any).addRibbonTabs([{
        header: { text: 'AI Assist', iconCss:"e-btn-icon e-icons e-assistview-icon e-icon-left" }, content: [
            {
...
public formulaValidate() {
  ...
  if (!isNullOrUndefined(selectedCell)) {
    (this.spreadsheet as any).getData(selectedCell).then((data: any) => {
          ...
          let query: string = 'Validate the below formulae and provide me the problem in it. Strictly provide the data for each validated response in a flat JSON array with fields `cell` to hold the spreadsheet cell value and `response` to hold the problem and solution as string.';
          ...
          if (isFormulaAvailable) {
              let aiOutput: any = getAzureChatAIRequest({ messages: [{ role: 'user', content: query }] });
              aiOutput.then((result: any) => {
                let cleanedResponseText: any = result;
                    if (result.indexOf('```json') > -1) {
                        ...
                        const startIndex: number = result.indexOf('['); 
                        const endIndex: number = result.indexOf(']') + 1;
                        cleanedResponseText = (cleanedResponseText as string).substring(startIndex, endIndex).trim();
                    }
...
public promptHandler(args: PromptRequestEventArgs): void {
  ...
  (this.spreadsheet as any).saveAsJson().then((data: any) => {
      const processedString: string = this.processDataSource(data);
      let query: string = prompt + '. Strictly provide the excel formula, within fenced code block with excel identifier, for the Excel sheet data which is provided as JSON below. /n' + processedString;
      let aiOutput: any = getAzureChatAIRequest({ messages: [{ role: 'user', content: query }] });
      aiOutput.then((result: any) => {
          if (result) {
            let cleanedResponseText: any = result;
            if (result.indexOf('```excel') > -1) {
                cleanedResponseText = result.split('```excel')[1].trim();
                cleanedResponseText = cleanedResponseText.split('```')[0].trim();
            }
