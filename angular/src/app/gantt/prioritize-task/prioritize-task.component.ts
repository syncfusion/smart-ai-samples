...
@Component({
  ...
  styleUrl: './prioritize-task.component.css'
})
export class PrioritizeTaskComponent {
  ...
  this.toolbar=[{template: '<button id="toolbarButton" class="e-btn e-primary">Assign prioritize tasks</button>', text:'Assign prioritize tasks' }]
}
...
public toolbarClick(args: any) {
  ...
  if (args.item.text === 'Assign prioritize tasks') {
    ...
    aioutput.then((result: any) => {
      ...
      Do not include any additional text or information. Also strictly return the JSON data in the format mentioned above.`
