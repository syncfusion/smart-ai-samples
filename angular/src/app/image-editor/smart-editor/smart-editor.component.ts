...
import { StabilityAiModelBGRemover,StabilityAiModel,StabilityAiModelMagicEraser } from '../../ai-models/stability-ai-models';
...
export class SmartEditorComponent implements OnInit{
  ...
  public treeData: { [key: string]: Object }[] = [
    ...
    { id: "3", name: "Remove Background", imageUrl: "images/remove-bg.png" }
  ];
  ...
  change(args: ColorPickerEventArgs): void {
    ...
    this.imageEditorObj.open('', false, {backgroundColor: this.colorPickerVal });
  }
  ...
  onCreated(): void {
    this.imageEditorObj.open('https://images.pexels.com/photos/103123/pexels-photo-103123.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1');
  }
  ...
  backgrounchanger(): void {
    ...
    if (this.outlineTextBox.value && this.outlineTextBox.value !== '') {
      ...
      let aiOutput = StabilityAiModel(file, prompt, searchPrompt);
      aiOutput.then((result) => {
        this.imageEditorObj.open(result, false, {backgroundColor: this.colorPickerVal });
        setTimeout(() => {
          this.bghideSpinner();
        }, 100);
        (document.getElementsByClassName('bg-changer')[0] as HTMLElement).style.display = 'none';
