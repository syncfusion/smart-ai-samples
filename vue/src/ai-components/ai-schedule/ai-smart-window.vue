<template>
    <div id="desc-container">
        <h2 style="text-align: center;">Smart Scheduling</h2>
        <div class="description-container e-card">
            <div class='e-card-content '>
                <p>The <b>Smart Scheduler</b> integrates with the Syncfusion Smart Paste Button Component, enabling users to
                    transform selected content directly into scheduler events. In the Smart Scheduler, the Smart Paste
                    Component intelligently analyzes the data and automatically creates corresponding events in the
                    scheduler, streamlining the scheduling process and saving time.</p>
                <p>Click the <b>Smart Appointment</b> button to create events based on the specified <b>Meeting
                        Contents</b>. Know more <a
                        href="https://github.com/syncfusion/smart-ai-samples/blob/master/vue/src/ai-components/ai-schedule/Readme.md">here</a>.
                </p>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        tabSelected: function (args: SelectEventArgs) {
            const toastObj = this.$refs.toastObj.ej2Instances;
            if ((args as any).selectedIndex === 1) {
                this.$refs.scheduleObj.ej2Instances.refresh();
                if (this.event && (this.event as any).Subject) {
                    toastObj.content = `${this.event.Subject} has been scheduled at ${this.event.StartTime.toLocaleString()}`;
                    toastObj.dataBind();
                    toastObj.show();
                }
        ...
        formSubmit: function (e: Event) {
            ...
            const dialogObj = this.$refs.dialogObj.ej2Instances;
            e.preventDefault();
            let subject = (document.getElementById('subject') as HTMLInputElement).value;
