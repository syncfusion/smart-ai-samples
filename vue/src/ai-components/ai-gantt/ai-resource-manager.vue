      ...
      <div class="description-container e-card">
        <div class="e-card-content">
          <p>This sample uses AI to prevent resource overallocation by reallocating tasks. It interacts with the
            ...
            redistributing tasks for balanced resource use. Taskbar colors are updated to reflect these changes,
            offering clear visual feedback on resource adjustments. Know more <a
              href="https://github.com/syncfusion/smart-ai-samples/blob/master/vue/src/ai-components/ai-gantt/Readme.md">here</a>.
          </p>
        </div>
      </div>
      <div id="container">
        <ejs-gantt id="GanttContainer" ref="ganttInstance" :dataSource="tasksCollection" :resources="resourceCollection"
          viewType="ResourceView" showOverAllocation="true" enableContextMenu="true" allowSorting="true"
          allowReordering="true" :taskFields="taskFields" :resourceFields="resourceFields" :editSettings="editSettings"
          :toolbar="toolbarOptions" @toolbarClick="toolbarClick" :labelSettings="labelSettings"
          :splitterSettings="splitterSettings" :selectionSettings="selectionSettings" :tooltipSettings="tooltipSettings"
          :timelineSettings="timelineSettings" readOnly="false" allowSelection="true" highlightWeekends="true"
          treeColumnIndex="1" taskbarHeight="20" rowHeight="40" height="550px">
          <e-columns>
...
export default {
  ...
  data() {
    return {
      ...
      toolbarOptions: [{
        template: this.ganttToolbarTemplate(),
  ...
  methods: {
    ganttToolbarTemplate() {
      let onToolbarButtonClick = this.onToolbarButtonClick;
      return function () {
        return {
          template: createApp().component('ganttToolbarTemplate', {
            template: `          <ejs-button id="toolbarButton" :isPrimary="true" @click="onToolbarButtonClick">
    ...
    onToolbarButtonClick() {
      ...
      aioutput.then((result) => {
        ...
        this.$refs.ganttInstance.ej2Instances.dataSource = JSON.parse(cleanedJsonData);
        this.$refs.ganttInstance.hideSpinner();
      });
    }
  },
