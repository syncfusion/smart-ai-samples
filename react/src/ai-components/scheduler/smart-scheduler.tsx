...
function SmartScheduler() {
    ...
    const AzureAIRequest = async (settings: ChatOptions) => {
        ...
        try {
            ...
            output =  response.replace('END_RESPONSE', '')
        } catch (error) {
    ...
    return (
        <>
            <div className="description-container e-card">
                <div className='e-card-header-title' style={{ fontWeight: 600 }}>Smart Paste Integration with Scheduler</div>
                <div className='e-card-content '>
                    <p>The <b>Smart Scheduler</b> integrates with the Syncfusion Smart Paste Button Component, enabling users to transform selected content directly into scheduler events. In the Smart Scheduler, the Smart Paste Component intelligently analyzes the data and automatically creates corresponding events in the scheduler, streamlining the scheduling process and saving time.</p>
                    <p>Click the <b>Schedule Appointment</b> button to create events based on the specified <b>Meeting Contents</b>. Know more <a href="https://github.com/syncfusion/smart-ai-samples/blob/master/react/src/ai-components/scheduler/Readme.md">here</a>.</p>
                </div>
            </div>
